<!DOCTYPE html>
<html>
<?php include ("header.php"); ?>
<body class="diretorias-inicial">
	<main>
		<h4 class="titulo">
			<span class="text">Goldfingers</span>
		</h4>
		<div class="conteudo container">
			<div class="row resumo">
				<div class="col-md-5 menus">
					<a><img src="assets/img/goldfingers.png" class="img-responsive"></a>
				</div>
				<div class="col-md-5 texto menus">
					<p>
						Também conhecido como Departamento de Captação de Recursos, os Goldfingers são
						responsáveis por levantar fundos para o GARRA, estabelecer parcerias e organizar eventos
						com fins de arrecadação. Além disso, eles também são responsáveis por atrair pessoas para o
						GARRA: membros, amigos, simpatizantes e patrocinadores. A diretoria do departamento é
						ocupada pela (o) Ministra (o) e os coordenadores são chamados de Goldfingers.
					</p>
				</div>
			</div>
			<div class="row membros">
				<ul>
					<li class="col-md-3 col-md-offset-2">
						<a><img src="assets/img/tayna.jpg" class="img-responsive img-circle"></a>
						<div class="descricao">
							<p><h4>Tayná Magalhães de Almeida</h4></p>
							<ul>
								<li>Curso: Medicina</li>
								<li>Cargo: Ministra do Goldfingers</li>
							</ul>
						</div>
					</li>
					<li class="col-md-3 col-md-offset-2">
						<a><img src="assets/img/ana-scafura.jpg" class="img-responsive img-circle"></a>
						<div class="descricao">
							<h4>Ana Luisa Scafura da Fonseca</h4>
							<ul>
								<li>Curso: Medicina</li>
								<li>Cargo: Goldfingers</li>
							</ul>
						</div>
					</li>
				</ul>
			</div>
			<div class="row membros">
				<ul>
					<li class="col-md-3 col-md-offset-2">
						<a><img src="assets/img/ana-luisa.jpg" class="img-responsive img-circle"></a>
						<div class="descricao">
							<h4>Ana Luisa Soares Costa</h4>
							<ul>
								<li>Curso: Medicina</li>
								<li>Cargo: Goldfingers</li>
							</ul>
						</div>
					</li>
					<li class="col-md-3 col-md-offset-2">
						<a><img src="assets/img/douglas.jpg" class="img-responsive img-circle"></a>
						<div class="descricao">
							<h4>Douglas de Castro Mendes</h4>
							<ul>
								<li>Curso: Engenharia Cívil</li>
								<li>Cargo: Goldfingers</li>
							</ul>
						</div>
					</li>
				</ul>
			</div>
			<div class="row membros">
				<ul>
					<li class="col-md-3 col-md-offset-2">
						<a><img src="assets/img/jessica-mendes.jpg" class="img-responsive img-circle"></a>
						<div class="descricao">
							<h4>Jéssica Mendes</h4>
							<ul>
								<li>Curso: Direito</li>
								<li>Cargo: Goldfingers</li>
							</ul>
						</div>
					</li>
				</ul>
			</div>
		</div>
	</main>
	<?php include ("footer.php"); ?>
</body>
</html>